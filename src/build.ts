...
export async function writeTypes(nitro: Nitro) {
  ...
  for (const mw of middleware) {
    ...
    o.request = `{`;
    if (imps.findIndex((i) => i.name === "validator") !== -1) {
      o.request += `body: Simplify<Serialize<ValidatorToX<typeof import('${relativePath}').validator, "body">>>,\n`;
      o.request += `query:Simplify<Serialize<ValidatorToX<typeof import('${relativePath}').validator, "query">>>,\n`;
      o.request += `params: Simplify<Serialize<ValidatorToX<typeof import('${relativePath}').validator, "params">>>,\n`;
    }
    o.request += `}`;
  ...
  const routes = [
    "// Generated by nitro",
    "import type { ValidatorToX } from '../../utils';",
    ...
    ...Object.entries(types.routes).map(([path, methods]) =>
      [
        ...
        ...Object.entries(methods).map(
          ([method, types]) =>
            `      '${method}': { response: Exclude<${types.map((v: any) => v.response).join(" | ")}, Error|void>, request: ${types.map((v: any) => v.request).join(" | ")}}` // Work around: I am too dumb to understand what types is
        ),
        "    }",
      ].join("\n")
    ),
